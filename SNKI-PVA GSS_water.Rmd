...
ggplot(kalman_wide, aes(x = time, y = `50%`, 
                        color = factor(region), 
                        fill = factor(region))) +
  facet_wrap(~wetland, scales = "free_y", ncol = 2) +
  geom_line() +
...
ggsave(filename = "Wetland_water_detectp.pdf",dpi = 300,
       width = 160, height = 230, units = "mm")
```

In this new plot, the open points represent three surveys per year (replicates): down-facing triangles are survey 4, circles are survey 5, and up-facing triangles are survey 6. This still do not include the extra parameter for detectability.