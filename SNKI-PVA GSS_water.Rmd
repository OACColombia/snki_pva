...
&=& \mathbf{A} + (\mathbf{I} + \mathbf{B})\mathbf{X}_t + \mathbf{E}_t&;&~\mathcal{B} = \mathbf{I}+ \mathbf{B}
\end{array}
$$
Note that the growth rate $\text{R}_t$ can be defined as:
$$
\begin{array}{cccc}
\text{R}_t&=&\ln{S_{t+1}}-\ln{S_t} && \\
&=& \mathbf{X}_{t+1} - \mathbf{X}_t &=& \mathbf{A} + \mathbf{BX}_t+\mathbf{E}_t \\ 

&=& \mathbf{A} + (\mathcal{B}-\mathbf{I})\mathbf{X}_t + \mathbf{E}_t
\end{array}
$$
In the long-run, $\mathbf{X}_t = \mathbf{X}_{\inf}$ is invariant. At stationary, $\mathbf{X}_{t+1} - \mathbf{X}_t = \mathbf{X}_{\inf}-\mathbf{X}_{\inf} = 0$, and $\text{R}_t = \mathbf{X}_{\inf}-\mathbf{X}_{\inf} = 0 =\mathbf{A} + (\mathcal{B}-\mathbf{I})\mathbf{X}_t + \mathbf{E}_t$.

Taking expected values, we get the average growth rate
$$
\begin{array}{ccc}
\text{E}[\text{R}_t] &=& \text{E}[\mathbf{A} + (\mathcal{B}-\mathbf{I})\mathbf{X}_{\inf} + \mathbf{E}_t]\\
&=&\mathbf{A} + (\mathcal{B}-\mathbf{I})\times \text{E}[\mathbf{X}_{\inf}]+0 \\
&=& \mathbf{A} + (\mathcal{B}-\mathbf{I})\mu

\end{array}
$$

Thus, this average growth rate is estimated with:
$$
\hat{\text{E}[\text{R}_t]} = \hat{\mathbf{A}}+(\hat{\mathcal{B}}-\mathbf{I})\hat{\mu}
$$

With a water covariate effect, the growth rate becomes:

$$
\mathbf{X}_{t+1} - \mathbf{X}_t = \mathbf{A} + \mathbf{BX}_t + \beta W_t^2 + \mathbf{E}_t
$$

Having estimated $\beta$ as $\hat{\beta}$, we can estimate the potential effect of water in the long-run growth rate as $\hat{\text{E}[\text{R}_t]} + \hat{\beta} W_t^2$, or the average modified growth rate with water effect. Set $\hat{\text{R*}}=\hat{\text{E}[\text{R}_t]} + \hat{\beta} W_t^2$. Then, the standardized water effect on the growth rate is estimated as

$$
\frac{\text{E}\hat{[\text{R}_t]}-\hat{\text{R*}}}{\text{E}[\hat{\text{R}_t}]} = \frac{\hat{\beta} W^2_t}{\text{E}\hat{[\text{R}_t]}}
$$

Extended to 
$$
\Delta_{\text{Growth rate}} = \frac{(\hat{\mathbf{A}}+(\hat{\mathcal{B}}-\mathbf{I})\hat{\mu})-(\hat{\mathbf{A}}+(\hat{\mathcal{B}}-\mathbf{I})\hat{\mu}+ \hat{\beta} W_t^2)}{\hat{\mathbf{A}}+(\hat{\mathcal{B}}-\mathbf{I})\hat{\mu}} 
$$

which is reduced to 
$$
\Delta\text{Growth rate}_{std} = \frac{\hat{\beta}\text{W}_t^2}{\hat{\mathbf{A}}+(\hat{\mathcal{B}}-\mathbf{I})\hat{\mu}} 
$$
...
plot(W.sim, std.gr.hat[,1], col=cool.palette[1],
     ylab = "Stnd. ∆ in growth rate", 
     xlab = "Relative water (stage)", 
     type = "l", 
     xlim = c(min(W),max(W)))
for(i in 2 :11){
  points(W.sim,std.gr.hat[,i], type="l", col=cool.palette[i])
}
...
ggsave(change.water.log.mu, filename = "Q2_log_mu.jpg",dpi = 300,
       width = 210, height = 180, units = "mm")
```
![Change in expected abundance (log-scale) with change in relative water (stage) values](Q2_log_mu.jpg)