...
The relationship of change in growth rate in response to water can be showed by the difference of growth rate including the quadratic water effect ($\mathbf{A}+\mathbf{BX}_t+\hat{\beta}\text{W}^2_t$) and growth rate without including the water effect (the average growth rate; $\mathbf{A}+\mathbf{BX}_t$):
$$
\Delta_{\text{Growth rate}} = \frac{(\mathbf{A}+(\mathcal{B}-\mathbf{I})\mathbf{X}_t+\hat{\beta}\text{W}^2_t)-(\mathbf{A}+(\mathcal{B}-\mathbf{I})\mathbf{X}_t)}{\mathbf{A}+(\mathcal{B}-\mathbf{I})\mathbf{X}_t} 
$$

which is reduced to 
$$
\Delta_{std}\text{Growth rate} = \frac{\hat{\beta}\text{W}^2}{\mathbf{A}+\mathbf{BX}_t} 
$$
evaluated at $\mathbf{X}_t=\hat{\mu}$
...
MVX.sim <- function(
  ...
  W <- (pt.change)^2 #change in value of Water, but quadratic effect
  p  <- theta.list$p
  Ip <- theta.list$Ip
...
But the previous figure is only one step, from the current year. If we want to compare across populations what is the effect, we can use as start the mean expected $\hat{\mu}$ from the overall trajectory
