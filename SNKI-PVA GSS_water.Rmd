...
mtext("Wetland level (ft)", side=1, cex=1.25, outer=TRUE)
mtext("Std. ∆ in resident population growth rate",side=2, cex=1.25, outer=TRUE)
...
wet.names <- gsub("[_]", " - ", wet.names)
wet.names <- gsub("[.]", " ", wet.names)
...
for(i in sensible){
  ...
  points(W.sim+mean_W4wetland[i], std.gr.hat[,i], col=sensi.palette[counter],
       type = "l", lwd=2)
  abline(v = mean_W4wetland[i], lty = 2, col = sensi.palette[counter])
  text(mean_W4wetland[i]+1.5, -0.8, round(mean_W4wetland[i], 1), col = sensi.palette[counter])
  counter <- counter + 1
}
mtext("Wetland water level (ft)", side=1, cex=1.25, outer=TRUE)
mtext("Std. ∆ in resident population growth rate",side=2, cex=1.25, outer=TRUE)
```
