...
mgssdclone_w <- dc.fit(data4dclone, 
                     ...
                     thin=thin, 
                     inits=myinits) 
saveRDS(mgssdclone_w, "Code/mgssdclone_w_p.RDS")
```
...
```{r}
mgssdclone_w <- readRDS("Code/mgssdclone_w_p.RDS")
...
```{r Figure at real scale with obs counts, warning=TRUE}
...
```
![Dynamic population of snail kites for 11 wetlands in 6 regions of Florida](Wetland_water_detectp.jpg)<br>

In this plot, the open points represent three surveys per year: down-facing triangles are survey 4, circles are survey 5, and up-facing triangles are survey 6. This dynamic includes the extra parameter for detectability and the influence of relative water sage.